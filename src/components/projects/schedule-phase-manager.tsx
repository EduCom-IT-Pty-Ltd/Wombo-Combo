...
export function SchedulePhaseManager({ projectId, phases, people, canManage }: { projectId: string; phases: SchedulePhaseView[]; people: Person[]; canManage: boolean }) {
  ...
  return <div className="space-y-4"><Card><CardHeader title="Project Call-Ups" description="Each Call-Up appears on the calendar for its assigned user." action={canManage ? <Button size="sm" variant="primary" onClick={() => setAdding(true)}><CalendarPlus className="size-4" /> Add Call-Up</Button> : null} />{adding ? <PhaseForm projectId={projectId} people={people} onClose={() => setAdding(false)} /> : null}{phases.length ? <div className="divide-y divide-border-subtle">{phases.map((phase) => <PhaseRow key={phase.id} phase={phase} projectId={projectId} people={people} canManage={canManage} />)}</div> : <EmptyState title="No Call-Ups scheduled" description={canManage ? "Add the first Call-Up to put this project on the calendar." : "No project Call-Ups have been scheduled yet."} />}</Card></div>;
}
