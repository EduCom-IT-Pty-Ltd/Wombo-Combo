...
function TemplateModal({ templates, statuses, onClose }: { templates: ProjectTemplate[]; statuses: StatusSetting[]; onClose: () => void }) {
  ...
  return createPortal(<div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 p-4" role="dialog" aria-modal="true" aria-label="Project templates"><div className="flex max-h-[90vh] w-full max-w-2xl flex-col overflow-hidden rounded-xl border border-border-strong bg-surface shadow-2xl"><div className="flex items-start justify-between gap-3 border-b border-border-subtle px-5 py-4"><div><h2 className="text-base font-bold">Project templates</h2><p className="mt-0.5 text-sm text-muted-foreground">Start new projects at the right workflow stage and complete earlier checklist tasks automatically.</p></div><Button size="sm" variant="ghost" aria-label="Close templates" onClick={onClose}><X className="size-4" /></Button></div><div className="overflow-y-auto"><form action={action} className="space-y-3 border-b border-border-subtle p-5"><p className="text-sm font-bold">New template</p><div className="grid gap-3 sm:grid-cols-2"><input required name="name" placeholder="Template name" className={inputClass} /><select required name="startingStatus" defaultValue="" className={inputClass}><option value="" disabled>Starting status…</option>{statuses.map((status) => <option key={status.status} value={status.status}>{status.label}</option>)}</select></div><input name="description" placeholder="Description (optional)" className={inputClass} /><div className="flex items-center gap-3"><Button type="submit" variant="primary" size="sm" disabled={pending}>{pending ? "Adding…" : "Add template"}</Button>{state.message ? <p className="text-xs text-muted-foreground">{state.message}</p> : null}</div></form><div className="divide-y divide-border-subtle">{templates.length ? templates.map((template) => <TemplateRow key={template.id} template={template} statuses={statuses} />) : <p className="px-5 py-8 text-center text-sm text-muted-foreground">No templates yet.</p>}</div></div></div></div>, document.body);
}
