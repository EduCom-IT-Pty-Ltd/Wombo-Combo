...
function ProjectEditor({ project, customers, onClose }: { project: ProjectDetail; customers: Array<{ id: string; name: string }>; onClose: () => void }) {
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4" role="dialog" aria-modal="true" aria-label="Edit project"><form action={action} className="flex max-h-[90vh] w-full max-w-2xl flex-col overflow-hidden rounded-xl border border-border-strong bg-surface shadow-2xl"><div className="flex items-start justify-between gap-3 border-b border-border-subtle px-5 py-4"><div><h2 className="text-base font-bold">Edit project</h2><p className="mt-0.5 text-sm text-muted-foreground">Update project details, including the generated Project ID.</p></div><Button type="button" size="sm" variant="ghost" aria-label="Close edit project" onClick={onClose}><X className="size-4" /></Button></div><div className="space-y-4 overflow-y-auto p-5"><input type="hidden" name="id" value={project.id} /><div className="grid gap-4 sm:grid-cols-2"><Field name="title" label="Project title" defaultValue={project.title} required /><Field name="projectNumber" label="Project ID" defaultValue={project.projectNumber} required /></div><label className="block"><span className="mb-1 block text-xs font-medium text-muted-foreground">Customer</span><select name="customerId" defaultValue={project.customerId} className={inputClass}>{customers.map((customer) => <option key={customer.id} value={customer.id}>{customer.name}</option>)}</select></label><div className="grid gap-4 sm:grid-cols-2"><Field name="siteName" label="Site name" defaultValue={project.site?.name ?? ""} /><Field name="contactName" label="Site contact" defaultValue={project.site?.accessNotes?.replace(/^Site contact:\s*/, "") ?? ""} /></div><div className="grid gap-4 sm:grid-cols-2"><Field name="requestedStartOn" label="Requested start" type="date" defaultValue={project.requestedStartOn ?? ""} /><Field name="poNumber" label="PO number" defaultValue={project.poNumber ?? ""} /></div><label className="block"><span className="mb-1 block text-xs font-medium text-muted-foreground">Scope of works</span><textarea name="scopeOfWorks" rows={5} defaultValue={project.scopeOfWorks ?? ""} className={`${inputClass} h-auto py-3`} /></label><label className="block"><span className="mb-1 block text-xs font-medium text-muted-foreground">Initial notes</span><textarea name="initialNotes" rows={3} defaultValue={project.initialNotes ?? ""} className={`${inputClass} h-auto py-3`} /></label></div><div className="flex flex-wrap items-center gap-3 border-t border-border-subtle px-5 py-4"><Button type="submit" variant="primary" disabled={pending}>{pending ? "Saving…" : "Save changes"}</Button><Button type="button" variant="ghost" onClick={onClose}>Cancel</Button>{state.message ? <p className={`text-xs ${state.ok ? "text-[var(--tone-emerald-fg)]" : "text-[var(--tone-rose-fg)]"}`}>{state.message}</p> : null}</div></form></div>;
}
