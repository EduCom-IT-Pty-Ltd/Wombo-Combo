...
export default async function NewCustomerPage() {
  ...
  const priceLists = await listCustomerPriceLists(session.org.id);
  return <div className="space-y-4"><Link href="/customers" className="inline-flex items-center gap-1 text-xs text-muted-foreground hover:text-foreground"><ArrowLeft className="size-3.5" /> Customers</Link><PageHeader title="Add customer" description="Create a customer to use on new project requests." /><NewCustomerForm priceLists={priceLists.map((list) => ({ id: list.id, name: list.name }))} /></div>;
}
