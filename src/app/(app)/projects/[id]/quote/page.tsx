...
export default async function ProjectQuotePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <div className="space-y-4">{can(session.role, "schedule.manage") ? <div className="flex justify-end"><AddOnsiteQuote projectId={project.id} people={people} /></div> : null}{can(session.role, "quote.edit") ? <MaterialQuoteBuilder projectId={project.id} materials={pricedMaterials} priceListName={priceList?.name ?? null} productionTemplates={productionTemplates} /> : null}{quotes.length ? <Card><CardHeader title="Generated quotes" description="Simplified materials quotes for this project." /> <div className="divide-y divide-border-subtle">{quotes.map((quote) => { const totals = calculateQuote(quote.lines, quote.taxRatePct); return <div key={quote.id} className="flex flex-wrap items-center justify-between gap-3 px-4 py-3"><div><p className="text-sm font-bold">{quote.reference}</p><p className="text-xs text-muted-foreground">{quote.lines.length} material lines · {formatMoney(totals.totalCents, session.org.currency)}</p></div><div className="flex flex-wrap items-center justify-end gap-2"><ButtonLink href={`/api/quotes/${quote.id}/pdf`} target="_blank" size="sm">Download PDF</ButtonLink>{can(session.role, "quote.edit") ? <QuoteActions projectId={project.id} quote={quote} materials={pricedMaterials} /> : null}</div></div>; })}</div></Card> : <Card><EmptyState title="No generated quotes" description="Add a production template, then set the material quantities to create the first quote." /></Card>}</div>;
}
