...
import { getMaterialCataloguePresentation, getProject, getXeroShortCode, listCatalogueMaterials, listCustomerPriceLists, listPeople, listQuotes } from "@/lib/data/repository";
...
export default async function ProjectQuotePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [quotes, materials, priceLists, presentation, people, xeroShortCode] = await Promise.all([listQuotes(session.org.id, id), listCatalogueMaterials(session.org.id), listCustomerPriceLists(session.org.id), getMaterialCataloguePresentation(session.org.id), listPeople(session.org.id), getXeroShortCode(session.org.id)]);
