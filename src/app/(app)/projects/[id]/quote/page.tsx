...
export default async function ProjectQuotePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [quotes, materials, priceLists] = await Promise.all([listQuotes(session.org.id, id), listCatalogueMaterials(session.org.id), listCustomerPriceLists(session.org.id)]);
  ...
  return <div className="space-y-4">{can(session.role, "quote.edit") ? <MaterialQuoteBuilder projectId={project.id} materials={pricedMaterials} priceListName={priceList?.name ?? null} /> : null}{quotes.length ? <Card><CardHeader title="Generated quotes" description="Simplified materials quotes for this project." /> <div className="divide-y divide-border-subtle">{quotes.map((quote) => { const totals = calculateQuote(quote.lines, quote.taxRatePct); return <div key={quote.id} className="flex flex-wrap items-center justify-between gap-3 px-4 py-3"><div><p className="text-sm font-bold">{quote.reference}</p><p className="text-xs text-muted-foreground">{quote.lines.length} material lines · {formatMoney(totals.totalCents, session.org.currency)}</p></div><ButtonLink href={`/api/quotes/${quote.id}/pdf`} target="_blank" size="sm">Download PDF</ButtonLink></div>; })}</div></Card> : <Card><EmptyState title="No generated quotes" description="Select materials and quantities above to create the first simplified quote." /></Card>}</div>;
}
