...
import { getOpenTimeEntry, listLeave, listPeople, listSchedulePhases, listTimeEntries } from "@/lib/data/repository";
...
export default async function FieldPage() {
  ...
  const [people, phases, leave, entries, openEntry] = await Promise.all([
    listPeople(session.org.id),
    listSchedulePhases(session.org.id, { userId }),
    listLeave(session.org.id, { userId }),
  ...
  return <div className="space-y-4"><div><h1 className="text-2xl font-bold tracking-tight">My field schedule</h1><p className="mt-1 text-sm text-muted-foreground">{formatDate(today, true)} · {me?.name ?? "Crew member"}</p></div><div className="grid grid-cols-2 gap-3"><Stat label="Today" value={`${hoursToday.toFixed(1)}h`} hint="Time logged" /><Stat label="Call-Ups" value={phases.length} hint="Assigned to you" /></div>{openEntry ? <Link href={`/field/${openEntry.projectId}`} className="flex min-h-14 items-center gap-3 rounded-xl border border-emerald-500/40 bg-emerald-500/10 px-4 transition-colors hover:bg-emerald-500/15"><Clock3 className="size-5 text-emerald-600" /><span className="min-w-0 flex-1"><span className="block text-sm font-bold">On site, checked in</span><span className="text-xs text-muted-foreground">Tap to return to the time clock</span></span><span className="text-xs font-bold text-emerald-700">Open</span></Link> : null}<PhaseCalendar phases={phases} people={me ? [me] : []} leave={leave} mode="field" title="My calendar" description="Your assigned project Call-Ups and unavailable periods. Tap a Call-Up to open its field job." onSiteAssignmentKeys={entries.filter((entry) => !entry.endedAt).map((entry) => `${entry.projectId}:${entry.userId}`)} /></div>;
}
