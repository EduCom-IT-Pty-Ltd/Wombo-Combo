...
export async function saveLabourSettings(_state: LabourActionState, formData: FormData): Promise<LabourActionState> {
  const session = await requireCapability("labour.manage");
  ...
  const value = { standardLabourEnabled: parsed.data.standardLabourEnabled, standardLabourCostCentsPerEmployee: Math.round(parsed.data.standardLabourCostPerEmployee * 100), subcontractorMaterialRates: parseRates(parsed.data.subcontractorRates) };
  if (hasDatabase) await pgSettings.saveLabourSettings(session.org.id, value);
  else await updateLocalLabourSettings(value);
...
export async function saveProjectCostingOptions(_state: LabourActionState, formData: FormData): Promise<LabourActionState> {
  ...
  const value = { standardLabourEnabled: parsed.data.standardLabourEnabled, employeeCount: parsed.data.employeeCount, includeSubcontractorMaterialCosts: parsed.data.includeSubcontractorMaterialCosts };
  if (hasDatabase) await pgSettings.saveProjectCostingOptions(session.org.id, parsed.data.projectId, value);
  else await updateLocalProjectCostingOptions(parsed.data.projectId, value);
